...
import { Button, Col, Nav, OverlayTrigger, Row, Tab, Tabs, Tooltip, TooltipProps } from "react-bootstrap";
...
const TooltipTopNavItem = ({tooltipMessage, eventKey, i, sI}: {tooltipMessage: any, eventKey: any, i: any, sI: any}) => {
  return (
    <OverlayTrigger
      key={`${eventKey}-top`}
      placement={'top'}
  ...
  >
    <Nav.Item>
      <Nav.Link eventKey={eventKey} disabled={disabled[i]}>{IconsByName(stagesIconsTypes[i], sI, "24px")}</Nav.Link>
    </Nav.Item>
  </OverlayTrigger>
  )
}
      ...
      <Row>
        <Col className="mb-6">
          ...
          {stagesIcons ? (
            ...
            <TooltipTopNavItem key={i} tooltipMessage={stagesTooltips[i]} eventKey={i} i={i} sI={sI} />
          ))) : (
            <></>
          )}
          </Nav>
